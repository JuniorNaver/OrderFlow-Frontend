...
async function fetchCategorySales(filters) {
    await mockDelay(320);
    return [
...
async function fetchInventoryStatus(filters) {
    await mockDelay(280);
    return [
        { name: "정상 재고", value: 62 },
...
// Using a simple context-less prop drill for clarity in the scaffold.
let _filters = null;
...
function KpiRow() {
    ...
    return (
        <section className="grid md:grid-cols-4 gap-4 mt-6">
            <KpiCard label="매출" value={`${currencyKRW(data.totalSales)}원`} hint="기간 총합" />
            <KpiCard label="거래건수" value={currencyKRW(data.orders)} hint="기간 총합" />
            <KpiCard label="객단가" value={`${currencyKRW(data.avgBasket)}원`} hint="매출/거래건수" />
            <KpiCard label="재고회전율" value={`${data.inventoryTurn.toFixed(1)}회`} hint="12개월 롤링" />
        </section>
    );
}
...
function SalesTrendCard() {
    ...
    return (
        <article className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4">
            <h2 className="text-base font-semibold">일자별 매출 & 예측</h2>
            <div className="h-[280px] mt-3">
                <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={data || []} margin={{ top: 10, right: 20, bottom: 0, left: 0 }}>
                        ...
                        <Legend />
                        <Line type="monotone" dataKey="sales" name="실매출" dot={false} strokeWidth={2} />
                        <Line type="monotone" dataKey="forecast" name="예측" dot={false} strokeDasharray="4 4" strokeWidth={2} />
                    </LineChart>
                </ResponsiveContainer>
            </div>
            {isLoading && <div className="text-sm text-slate-400">로딩 중…</div>}
        </article>
    );
}
...
function CategorySalesCard() {
    ...
    return (
        <article className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4">
            <h2 className="text-base font-semibold">카테고리별 매출</h2>
            <div className="h-[280px] mt-3">
                <ResponsiveContainer width="100%" height="100%">
...
function InventoryStatusCard() {
    ...
    const total = useMemo(() => (data ? data.reduce((a, b) => a + b.value, 0) : 0), [data]);

    return (
        <article className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4">
            <h2 className="text-base font-semibold">재고 상태 분포</h2>
            <div className="h-[280px] mt-3">
                <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                        <Pie dataKey="value" nameKey="name" data={data || []} outerRadius={90} label>
                            {(data || []).map((_, i) => (
                                <Cell key={i} />
                            ))}
                        </Pie>
                        <Legend />
                        <Tooltip formatter={(v) => `${v}건 (${total ? Math.round((v / total) * 100) : 0}%)`} />
                    </PieChart>
                </ResponsiveContainer>
            </div>
            {isLoading && <div className="text-sm text-slate-400">로딩 중…</div>}
...
function NotesCard() {
    return (
        <article className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4">
            <h2 className="text-base font-semibold">메모 & 액션</h2>
            ...
            <div className="mt-3 flex gap-2">
                <button className="px-3 py-2 rounded-xl border hover:shadow text-sm" onClick={() => alert("발주 추천 (TODO)")}>발주 추천</button>
                <button className="px-3 py-2 rounded-xl border hover:shadow text-sm" onClick={() => alert("보고서 내보내기 (TODO)")}>보고서 내보내기</button>
            </div>
        </article>
    );
}
